...
public class WeatherModel {
  ...
  public static String getWeatherForCity(String city, int days) {
    try {
      ...
      double latitude = Double.parseDouble(body.substring(latIndex + 11, body.indexOf(",", latIndex)).trim());
      double longitude = Double.parseDouble(body.substring(lonIndex + 12, body.indexOf(",", lonIndex)).trim());
      ...
      return weatherResponse.body(); // Return raw JSON forecast
  ...
  public static WeatherRecommendation getRecommendedDays(String city, int days) {
    ...
    try {
      String[] dates = forecast.split("\"time\":\\[")[1].split("]")[0].replace("\"", "").split(",");
      String[] codes = forecast.split("\"weathercode\":\\[")[1].split("]")[0].split(",");
      ...
      String message;
      if (niceDays.isEmpty()) {
        message = "No clear days in the next " + days + " days for " + city + ".";
      } else {
        message = "These days are expected to have clear weather in " + city + "!";
      }

      return new WeatherRecommendation(city, niceDays, message);

    } catch (Exception e) {
      return new WeatherRecommendation(city, List.of(), "Error processing forecast: " + e.getMessage());
    }
  }
