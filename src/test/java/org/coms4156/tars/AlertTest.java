...
public class AlertTest {
    ...
    void setUp() {
        ...
        mockWeatherAlert = new WeatherAlert("New York", mockAlerts, mockRecommendations, mockCurrentConditions);
    }


    @Test
    void testGetWeatherAlertsWithValidCity() throws Exception {
        try (MockedStatic<WeatherAlertModel> mockedModel = Mockito.mockStatic(WeatherAlertModel.class)) {
            mockedModel.when(() -> WeatherAlertModel.getWeatherAlerts("New York", null, null))
            ...
            mockMvc.perform(get("/alert/weather")
                    ...
                    .andExpect(jsonPath("$.currentConditions.temperature_celsius").value(22.5));
        }
    }
    ...
    void testGetWeatherAlertsWithValidCoordinates() throws Exception {
        try (MockedStatic<WeatherAlertModel> mockedModel = Mockito.mockStatic(WeatherAlertModel.class)) {
            WeatherAlert coordAlert = new WeatherAlert("40.7128, -74.0060", mockAlerts, mockRecommendations, mockCurrentConditions);
            mockedModel.when(() -> WeatherAlertModel.getWeatherAlerts(null, 40.7128, -74.0060))
    ...
    void testGetWeatherAlertsWithCityContainingSpaces() throws Exception {
        try (MockedStatic<WeatherAlertModel> mockedModel = Mockito.mockStatic(WeatherAlertModel.class)) {
            WeatherAlert spacedAlert = new WeatherAlert("San Francisco", mockAlerts, mockRecommendations, mockCurrentConditions);
            mockedModel.when(() -> WeatherAlertModel.getWeatherAlerts("San Francisco", null, null))
    ...
    void testGetWeatherAlertsWithAccentedCityName() throws Exception {
        try (MockedStatic<WeatherAlertModel> mockedModel = Mockito.mockStatic(WeatherAlertModel.class)) {
            WeatherAlert accentedAlert = new WeatherAlert("São Paulo", mockAlerts, mockRecommendations, mockCurrentConditions);
            mockedModel.when(() -> WeatherAlertModel.getWeatherAlerts("São Paulo", null, null))
    ...
    void testGetWeatherAlertsWithExtremeCoordinates() throws Exception {
        try (MockedStatic<WeatherAlertModel> mockedModel = Mockito.mockStatic(WeatherAlertModel.class)) {
            WeatherAlert extremeAlert = new WeatherAlert("90.0000, 180.0000", mockAlerts, mockRecommendations, mockCurrentConditions);
            mockedModel.when(() -> WeatherAlertModel.getWeatherAlerts(null, 90.0, 180.0))
